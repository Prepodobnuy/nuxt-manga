...
async def get_user_pfp(uuid: str) -> FileResponse:
    user: User = User.get_by_uuid(session, uuid)
    if user is None: raise HTTPException(404)
    if user.uhd_path is None: return FileResponse(DEFAULT_PFP)
...
async def get_user_pfp(uuid: str) -> FileResponse:
    user: User = User.get_by_uuid(session, uuid)
    if user is None: raise HTTPException(404)
    if user.hd_path is None: return FileResponse(DEFAULT_PFP)
