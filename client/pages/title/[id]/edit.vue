...
import { EntityPersonEntry, UiImageInput, UiInput } from "#components";
import type { PersonFull } from "~/types/person";
import { useEditTitleForm } from "~/forms/useEditTitleForm";
...
const images = () =>
  store.form.cover_ids.map((v) => {
    return {
      url: `${config.public.apiBase}/api/asset/title/${id}/cover/${v}`,
      id: v,
    };
  });
...
const addCover = () => {
  let index =
    store.form.cover_ids.length > 0 ? Math.max(...store.form.cover_ids) + 1 : 0;
  store.form.cover_ids.push(index);
};

const cover = `${config.public.apiBase}/api/asset/title/${id}/cover`;

const select_image = async (id: number, image: File) => {
  await store.putTitleCover(id, image);
};

const { form, valid } = storeToRefs(useEditTitleForm(id));
const { clear, post } = useEditTitleForm(id);
const { tags, genres } = useTags();

const tagEntries = computed(() => {
  return tags
    .filter((t) => !form.value.tags.includes(t.id))
    .map((t) => ({
      display: t.ru,
      select: t.id,
    }));
...
const genreEntries = computed(() => {
  return genres
    .filter((g) => !form.value.genres.includes(g.id))
    .map((g) => ({
      display: g.ru,
      select: g.id,
    }));
...
<template>
  <LayoutPseudoHeader>
    ...
    <div class="content">
      <UiFormBlock @click="valid.covers = true">
        <h6>Обложки</h6>
        <UiButton
          @click="addCover()"
          color="success"
